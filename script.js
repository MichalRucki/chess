...
const checkPawnButtons = document.querySelector('.checkPawn')

const buttonKing = document.getElementById('buttonKing')
const buttonRock = document.getElementById('buttonPawn')
...
function checkPawn(newY, newX) {
    for (let i = 0; i < 8; i++) {
        if (fields[0][i].symbol === pawn || fields[7][i].symbol === pawn) {
            checkPawnButtons.style.display = 'flex'
        }
    }
    
    buttonKing.addEventListener('click', () => {
        fields[newY][newX].symbol = king
        checkPawnButtons.style.display = 'none'
        generateFields()
    })
    buttonBishop.addEventListener('click', () => {
        fields[newY][newX].symbol = bishop
        checkPawnButtons.style.display = 'none'
        generateFields()
    })
    buttonKnight.addEventListener('click', () => {
        fields[newY][newX].symbol = knight
        checkPawnButtons.style.display = 'none'
        generateFields()
    })
    buttonRock.addEventListener('click', () => {
        fields[newY][newX].symbol = rock
        checkPawnButtons.style.display = 'none'
        generateFields()
    })
}
